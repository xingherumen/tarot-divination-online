    ...
    TS.SpreadEngine = {
        ...
        saveToHistory: function () {
            if (!this.currentResult) {
                StorageLogger.warn('saveToHistory: currentResult 为空');
                return;
            }

            var record = {
                id: this.currentResult.id || (Date.now() + '_spread'),
                date: this.currentResult.timestamp || new Date().toLocaleString(),
                type: 'spread',
                ...
                cards: this.currentResult.cards.map(function (item) {
                    return {
                        ...
                        cardName: item.card ? item.card.name : '',
                        orientation: item.orientation,
                        meaning: item.orientation === '正位' ? (item.card ? item.card.meaning : '') : (item.card ? item.card.reverse : '')
                    };
                }),
            ...
            if (typeof HistoryStore !== 'undefined' && HistoryStore.add) {
                ...
                try {
                    ...
                    if (typeof StorageLogger !== 'undefined') {
                        StorageLogger.error('saveToHistory: localStorage 写入失败', e);
                    }
                }
            }
