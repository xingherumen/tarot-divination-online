(function () {
    ...
    var StorageLogger = {
        ...
        info: function (msg, data) {
            if (data) console.log(this.prefix, msg, data);
            else console.log(this.prefix, msg);
        },
        warn: function (msg, data) {
            if (data) console.warn(this.prefix, msg, data);
            else console.warn(this.prefix, msg);
        },
        error: function (msg, data) {
            if (data) console.error(this.prefix, msg, data);
    ...
    var HistoryStore = {
        getAll: function () {
            try {
                ...
                StorageLogger.info('getAll: 获取 ' + data.length + ' 条记录');
                return data;
            } catch (e) {
                StorageLogger.error('getAll 失败', e);
                return [];
            }
        },
        ...
        add: function (record) {
            if (!record) {
                StorageLogger.warn('add: 记录为空，跳过保存');
                return false;
            }
            if (!record.id) {
                record.id = Date.now() + '_' + Math.random().toString(36).substr(2, 6);
            }
            if (!record.date) {
            ...
            if (history.length > CONFIG.MAX_HISTORY) {
                ...
                StorageLogger.info('add: 超出上限，截断至 ' + CONFIG.MAX_HISTORY + ' 条');
            }

            try {
                var jsonStr = JSON.stringify(history);
                if (jsonStr.length > 5000000) {
                    StorageLogger.warn('add: 存储数据过大 (' + (jsonStr.length / 1024 / 1024).toFixed(1) + 'MB)，尝试保存');
                }
                localStorage.setItem(CONFIG.STORAGE_KEY, jsonStr);
                StorageLogger.info('add: 保存成功 - ' + (record.cardName || record.spreadName || record.id));
                return true;
            } catch (e) {
                StorageLogger.error('add: localStorage 写入失败', e);
                if (e.name === 'QuotaExceededError' || e.code === 22) {
                    ...
                    try {
                        ...
                        StorageLogger.info('add: 清理后重试保存成功，保留 ' + history.length + ' 条');
                        return true;
                    } catch (e2) {
                        StorageLogger.error('add: 清理后仍无法保存', e2);
                        return false;
                    }
                }
                return false;
        ...
        clear: function () {
            try {
                ...
                StorageLogger.info('clear: 已清空所有记录');
            } catch (e) {
                StorageLogger.error('clear 失败', e);
            }
        },
        ...
        removeById: function (id) {
            try {
                var history = this.getAll();
                var filtered = history.filter(function (r) { return r.id !== id; });
                if (filtered.length < history.length) {
                    localStorage.setItem(CONFIG.STORAGE_KEY, JSON.stringify(filtered));
                    StorageLogger.info('removeById: 已删除记录 ' + id);
                    return true;
                }
                return false;
            } catch (e) {
                StorageLogger.error('removeById 失败', e);
                return false;
            }
        }
    };
    ...
    var DrawDeckEngine = {
        ...
        showMultiResult: function () {
            ...
            // 保存多抽记录到历史
            var multiDrawSaved = false;
            try {
                var multiRecord = {
                    id: Date.now() + '_multi',
                    date: new Date().toLocaleString(),
                    type: 'multi_draw',
                    cardCount: self.multiDrawn.length,
                    cards: self.multiDrawn.map(function (entry) {
                        return {
                            cardName: entry.card.name,
                            orientation: entry.orientation,
                            meaning: entry.orientation === '正位' ? entry.card.meaning : entry.card.reverse,
                            advice: entry.card.advice,
                            element: entry.card.element,
                            planet: entry.card.planet
                        };
                    }),
                    stats: {
                        ...
                        total: self.multiDrawn.length
                    },
                    overallAdvice: overallAdvice
                };
                if (typeof HistoryStore !== 'undefined' && HistoryStore.add) {
                    multiDrawSaved = HistoryStore.add(multiRecord);
                }
            } catch (e) {
                StorageLogger.error('showMultiResult: 保存多抽记录失败', e);
            }
            ...
            ra.innerHTML =
                '<div class="multi-result-interp">' +
                    ...
                    '</div>' +
                    (multiDrawSaved
                        ? '<div class="save-status success">✅ 抽牌记录已自动保存</div>'
                        : '<div class="save-status"><button class="btn btn-save" id="saveMultiDrawBtn">💾 手动保存记录</button></div>') +
            ...
            var saveMultiBtn = document.getElementById('saveMultiDrawBtn');
            if (saveMultiBtn) {
                saveMultiBtn.onclick = function () {
                    if (typeof HistoryStore !== 'undefined' && HistoryStore.add) {
                        var saved = HistoryStore.add({
                            id: Date.now() + '_multi_manual',
                            date: new Date().toLocaleString(),
                            type: 'multi_draw',
                            cardCount: self.multiDrawn.length,
                            cards: self.multiDrawn.map(function (entry) {
                        ...
                        if (saved) {
                            saveMultiBtn.textContent = '✅ 已保存';
                            saveMultiBtn.disabled = true;
                        } else {
                            saveMultiBtn.textContent = '❌ 保存失败，重试';
                        }
                    }
                };
            }
    ...
    // ==================== 历史页面初始化 ====================
    function buildHistoryItemHTML(record) {
        if (record.type === 'spread') {
            var spreadCards = (record.cards || []).map(function (c) {
                return c.cardName + ' ' + c.orientation;
            }).join('、');
            var truncatedInterp = record.interpretation && record.interpretation.overall
                ? (record.interpretation.overall.length > 80
                    ? record.interpretation.overall.substring(0, 80) + '...'
                    : record.interpretation.overall)
                : '';
            return '<div class="history-item history-spread">' +
                '<div class="history-type-badge spread">🔮 牌阵</div>' +
                '<div class="date">' + record.date + '</div>' +
                '<div class="card-name">📋 ' + (record.spreadName || '牌阵占卜') + '</div>' +
                (record.question ? '<div class="question">💭 ' + record.question + '</div>' : '') +
                '<div class="meaning">牌：' + spreadCards + '</div>' +
                (truncatedInterp ? '<div class="interp-preview">📖 ' + truncatedInterp + '</div>' : '') +
            '</div>';
        }

        if (record.type === 'multi_draw') {
            var multiCards = (record.cards || []).map(function (c) {
                return c.cardName + ' ' + c.orientation;
            }).join('、');
            return '<div class="history-item history-multi">' +
                '<div class="history-type-badge multi">🎴 多抽</div>' +
                '<div class="date">' + record.date + '</div>' +
                '<div class="card-name">📋 多抽占卜（' + (record.cardCount || record.cards.length) + '张）</div>' +
                '<div class="meaning">牌：' + multiCards + '</div>' +
                (record.overallAdvice ? '<div style="font-size: 0.7rem; color: rgba(255,255,255,0.4); margin-top: 8px;">💫 ' + record.overallAdvice + '</div>' : '') +
            '</div>';
        }
        ...
        var truncated = (record.meaning || '').length > 60
            ...
            : record.meaning || '';
        return '<div class="history-item history-single">' +
            '<div class="history-type-badge single">🃏 单抽</div>' +
            '<div class="date">' + record.date + '</div>' +
            '<div class="card-name">🔮 ' + (record.cardName || '未知') + ' ' + (record.orientation || '') + '</div>' +
            (truncated ? '<div class="meaning">「' + truncated + '」</div>' : '') +
            (record.advice ? '<div style="font-size: 0.7rem; color: rgba(255,255,255,0.4); margin-top: 8px;">📜 ' + record.advice + '</div>' : '') +
        '</div>';
    }
    ...
    function renderHistoryList() {
        ...
        historyList.innerHTML = history.map(function (record) {
            return buildHistoryItemHTML(record);
        }).join('');
    }
